...
import { Medal, Palette, PenTool } from "lucide-react";
...
export default function RorschachClassementPage() {
  const [scores, setScores] = useState<PlayerScore[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/rorschach-scores")
      .then((res) => res.json())
      ...
      <div className="max-w-2xl w-full">
        ...
        {loading ? (
          ...
          <div className="card text-center py-12 animate-slide-up">
            <div className="flex justify-center mb-4"><PenTool size={48} className="text-black" /></div>
        ...
        <div className="text-center mt-10">
          <Link href="/rorschach" className="btn-secondary inline-block">
            Retour au test
          </Link>
        </div>
      </div>
